#include <Arduino.h>
#include "model_parameters_quantize_only.h"
#define INPUT_SIZE 3
...
// int8をfloatに復元する関数
float dequantize(int8_t q_value, float scale) {
...
void predict(float input[INPUT_SIZE], float output[OUTPUT_SIZE]) {
    ...
    // **Layer 1 計算** (量子化されたweightを使用、scaleで復元)
    for (int i = 0; i < HIDDEN_SIZE1; i++) {
        layer1[i] = dequantize(bias_1[i], bias_1_scale);
        for (int j = 0; j < INPUT_SIZE; j++) {
          float weight_val = dequantize(weight_1[i * INPUT_SIZE + j], weight_1_scale);
          layer1[i] += weight_val * input[j];
        }
        layer1[i] = relu(layer1[i]); // ReLU
    }

    // **Layer 2 計算** (量子化されたweightを使用、scaleで復元)
    for (int i = 0; i < HIDDEN_SIZE2; i++) {
        layer2[i] = dequantize(bias_2[i], bias_2_scale);
        for (int j = 0; j < HIDDEN_SIZE1; j++) {
            float weight_val = dequantize(weight_2[i * HIDDEN_SIZE1 + j], weight_2_scale);
            layer2[i] += weight_val * layer1[j];
        }
        layer2[i] = relu(layer2[i]); // ReLU
    }

    // **Layer 3（出力層）計算** (量子化されたweightを使用、scaleで復元)
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        output[i] = dequantize(bias_3[i], bias_3_scale);
        for (int j = 0; j < HIDDEN_SIZE2; j++) {
            float weight_val = dequantize(weight_3[i * HIDDEN_SIZE2 + j], weight_3_scale);
            output[i] += weight_val * layer2[j];
        }
    }
}
