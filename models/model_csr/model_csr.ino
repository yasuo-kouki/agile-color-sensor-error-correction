#include <Arduino.h>
#include "model_parameters_csr_only.h"
#define INPUT_SIZE 3
...
// CSR形式の行列とベクトルの積を計算する関数
void csr_matrix_vector_multiply(float* data, int* indices, int* indptr, int rows, int cols, 
                                 float* input, float* output) {
    for (int i = 0; i < rows; i++) {
        output[i] = 0.0;
        for (int j = indptr[i]; j < indptr[i + 1]; j++) {
            output[i] += data[j] * input[indices[j]];
        }
    }
}
...
void predict(float input[INPUT_SIZE], float output[OUTPUT_SIZE]) {
    ...
    // **Layer 1 計算** (CSR形式のweightを使用)
    csr_matrix_vector_multiply(weight_1_data, weight_1_indices, weight_1_indptr, 
                               weight_1_shape[0], weight_1_shape[1], 
                               input, layer1);
    // biasを加算
    ...
    // **Layer 2 計算** (CSR形式のweightを使用)
    csr_matrix_vector_multiply(weight_2_data, weight_2_indices, weight_2_indptr, 
                               weight_2_shape[0], weight_2_shape[1], 
                               layer1, layer2);
    // biasを加算
    ...
    // **Layer 3（出力層）計算** (CSR形式のweightを使用)
    csr_matrix_vector_multiply(weight_3_data, weight_3_indices, weight_3_indptr, 
                               weight_3_shape[0], weight_3_shape[1], 
                               layer2, output);
    // biasを加算
