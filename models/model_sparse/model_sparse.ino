#include <Arduino.h>
#include "model_parameters_sparse_only.h"
#define INPUT_SIZE 3
...
void predict(float input[INPUT_SIZE], float output[OUTPUT_SIZE]) {
    ...
    // **Layer 1 計算** (スパース化されたweightを使用、float型のまま)
    ...
    // **Layer 2 計算** (スパース化されたweightを使用、float型のまま)
    ...
    // **Layer 3（出力層）計算** (スパース化されたweightを使用、float型のまま)
